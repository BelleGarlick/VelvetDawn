...
public class TestTurns extends BaseTest {
    ...
    public void test_begin_next_turn_resets_entity_movement() throws Exception {
        ...
        velvetDawn.game.startSetupPhase();
        velvetDawn.game.setup.placeEntity(p1, "civil-war:commander", new Coordinate(0, -this.getConfig().map.borderRadius));
        ...
        assertEquals(
            unit.attributes.get("movement.remaining"),
    ...
    public void test_begin_next_turn_and_get_active_turn() throws Exception {
        ...
        var p1 = velvetDawn.players.join("player1", "password");
        var p2 = velvetDawn.players.join("player2", "password");
        assertNotNull(velvetDawn.game.turns.getActiveTurn());
    ...
    public void test_update_turn_start_time() throws Exception {
        ...
        assertNull(velvetDawn.game.turns.currentTurnData().getTurnStart());
    ...
    @Test
    public void test_current_turn_time() {
        var config = this.getConfig();
        var velvetDawn = new VelvetDawn(config);

        config.turnTime = 10;
        assertEquals(10000, velvetDawn.game.turns.currentTurnTime());

        config.turnTime = 500;
        assertEquals(500000, velvetDawn.game.turns.currentTurnTime());

        // Test setup is always 5 mins
        config.setupTime = 5;
        velvetDawn.game.phase = Phase.Setup;
        assertEquals(5000, velvetDawn.game.turns.currentTurnTime());
    }
}
