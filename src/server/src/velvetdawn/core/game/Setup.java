...
public class Setup {
     ...
     * the lobby phase.
     *
     * @param datapackId The id of the entity to update the
     *                 count for
     * @param count The number of entities of this type
     *              a player may use. For commanders, this
     *              specific number will be ignored.
     */
    public void updateSetup(String datapackId, Integer count) throws Exception {
        if (velvetDawn.game.phase != Phase.Lobby)
            throw new Exception("Game setup units may only be changed in the lobby by the admin");

        // Check entity exists
        if (!velvetDawn.datapacks.entities.containsKey(datapackId))
            throw new Exception(String.format("Unknown entity id: '%s'", datapackId));

        if (count == 0) {
            this.setup.remove(datapackId);
        } else
